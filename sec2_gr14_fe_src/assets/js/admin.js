/*
    admin.js — Administrative Hub & Inventory Management
    Purpose: Orchestrates the secure admin environment, including the 
             dynamic product table, parametric metadata management, 
             and real-time dashboard analytics.
    Dependencies: BSC (api.js) must be initialized for service communication.
*/
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (adminTableBody) {
        ...
        // POLLED UPDATES: Keep stock and stats fresh every 10s
        setInterval(() => {
            refreshAdminData();
        }, 10000);
    }
    ...
    async function loadAdminProducts(filters = {}) {
        ...
        try {
            ...
            adminTableBody.innerHTML = products.map(p => {
                ...
                const stockColor = stockVal < 5 ? '#e11d48' : '#6b7280';
    ...
    function collectFilters() {
        ...
        const filters = {};
        if (categories.length > 0) filters.category = categories[0]; 
        if (colors.length > 0) filters.colorId = colors[0]; 
    ...
    async function bindAddProductForm() {
        ...
        async function prefillEditForm(id) {
            try {
                ...
                if (inputs[2]) inputs[2].value = product.QuantityLeft || 0;
                ...
                if (dimensions[0]) dimensions[0].value = product.LengthDimension || product.LengthCM || 0;
                if (dimensions[1]) dimensions[1].value = product.WidthDimension || product.WidthCM || 0;
                if (dimensions[2]) dimensions[2].value = product.HeightDimension || product.HeightCM || 0;
                if (dimensions[3]) dimensions[3].value = product.Weight || product.WeightKG || 0;
